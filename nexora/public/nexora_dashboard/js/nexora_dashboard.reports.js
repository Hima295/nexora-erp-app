...
window.NexoraDashboard.Reports = (function () {
    ...
    function emptyPrefs() {
        return { pinned: [], recent: [], saved: [], favorites: [], shared: [], custom: [], settings: { show_empty: true, compact: false } };
    }
    ...
    function readPrefs() {
        try {
            ...
            return {
                ...
                settings: Object.assign({ show_empty: true, compact: false }, o.settings || {})
            };
    ...
    function viewWrap(inner) {
        return '<div class="nx-view">' + inner + "</div>";
    }

    function loadingHtml(app, label) {
        return '<div class="nx-report-loading">' + (app.ic ? app.ic("refresh", 20) : "") + " <span>" + app.esc(label) + "</span></div>";
    }
    ...
    function renderHub(app, cat, rx) {
        ...
        if (app.pageHeader) {
            html += app.pageHeader({
                ...
                title: app.t("Reports Center"),
                subtitle: app.t("Nexora-native reports — live data, rendered by Nexora"),
                crumb: [app.t("Nexora"), app.t("Reports Center")]
            });
        }

        var cats = renderCategories(app, cat, p);

        html += '<div class="nx-rx-center">' +
            renderSidebar(app, cat, p) +
            '<div class="nx-rx-main">' +
            renderToolbar(app, cat, p) +
            cats.html +
            "</div></div>";

        app.main.innerHTML = viewWrap(html);
        var count = app.main.querySelector("[data-rx-count]");
        if (count) count.textContent = app.num(cats.total, 0) + " " + app.t("reports");
        bindSearch(app, rx);
    }
    ...
    function catSectionHtml(app, c, reps) {
        return '<section class="nx-rx-cat" data-rx-cat="' + app.esc(c.key) + '">' +
            '<header class="nx-rx-cat-head">' +
            '<span class="nx-rx-cat-ic nx-ic-' + app.esc(c.color || "indigo") + '">' + app.ic(c.icon || "chart", 18) + "</span>" +
            '<div class="nx-rx-cat-txt"><div class="nx-rx-cat-title">' + app.esc(app.t(c.label || c.key)) + "</div>" +
            '<div class="nx-rx-cat-desc">' + app.esc(app.t(c.desc || "")) + "</div></div>" +
            '<span class="nx-badge nx-badge-' + app.esc(c.color || "indigo") + '">' + app.num(reps.length, 0) + " " + app.t("reports") + "</span>" +
            "</header>" +
    ...
    function renderReportCards(app, c, reps) {
        if (!reps.length) {
            return '<div class="nx-rx-cat-empty"><span class="nx-rx-cat-empty-ic">' + app.ic("plus", 14) + "</span>" +
                app.esc(app.t("No reports in this category yet — they appear here once published.")) + "</div>";
        }
        var html = "";
        reps.forEach(function (it) {
            html += it.kind === "custom" ? customCardHtml(app, it.r) : reportCardHtml(app, it.c, it.r);
        });
        return html;
    }
    ...
    function renderSidebar(app, cat, p) {
        ...
        return '<aside class="nx-rx-sidebar">' +
            '<div class="nx-rx-side-label">' + app.esc(app.t("LIBRARY")) + "</div>" +
            '<nav class="nx-rx-side-nav">' +
            renderRecent(app, recent) +
            renderPinned(app, pinned) +
            renderFavorites(app, favorites) +
            renderSaved(app, p.saved) +
            renderShared(app, shared) +
            "</nav></aside>";
    }

    function renderRecent(app, recent) {
        return sideSectionHtml(app, app.t("Recent Reports"), "clock",
            recent.map(function (r) { return { key: r.key, title: r.title, ts: r.ts }; }),
            "recent", !!recent.length);
    }

    function renderPinned(app, pinned) {
        return sideSectionHtml(app, app.t("Pinned Reports"), "pin",
            pinned.map(function (k) { return { key: k, title: reportMetaFor(app, k).title, ts: 0 }; }),
            "pinned", !!pinned.length);
    }

    function renderFavorites(app, favorites) {
        return sideSectionHtml(app, app.t("Favorites"), "heart",
            favorites.map(function (k) { return { key: k, title: reportMetaFor(app, k).title, ts: 0 }; }),
            "favorites", !!favorites.length);
    }

    function renderSaved(app, saved) {
        return sideSectionHtml(app, app.t("Saved Reports"), "save",
            saved.map(function (s) { return { key: s.key, title: s.title || s.key, ts: s.ts }; }),
            "saved", !!saved.length);
    }

    function renderShared(app, shared) {
        return sideSectionHtml(app, app.t("Shared Reports"), "share",
            shared.map(function (s) { return { key: s.key, title: s.title || s.key, ts: s.ts }; }),
            "shared", !!shared.length);
    }

    function sideSectionHtml(app, label, icon, items, kind, hasItems) {
        var html = '<section class="nx-rx-side-sec">' +
            '<header class="nx-rx-side-sec-head"><span>' + app.ic(icon, 13) + " " + app.esc(label) + "</span>" +
            (hasItems ? '<button class="nx-icon-btn" data-rx-clear="' + app.esc(kind) + '" title="' + app.esc(app.t("Clear")) + '">' + app.ic("x", 12) + "</button>" : "") +
            "</header>" +
    ...
    function renderToolbar(app, cat, p) {
        return '<div class="nx-rx-toolbar">' +
            '<div class="nx-hub-search"><span class="nx-search-ic">' + app.ic("search", 14) + '</span>' +
            '<input class="nx-input nx-hub-filter nx-rx-search" type="text" autocomplete="off" spellcheck="false" placeholder="' + app.esc(app.t("Search reports…")) + '" data-rx-search />' +
            "</div>" +
            '<span class="nx-hub-count" data-rx-count></span>' +
            '<span class="nx-rx-toolbar-spacer"></span>' +
            '<button class="nx-btn nx-btn-primary nx-btn-sm" data-rx-tool="new" title="' + app.esc(app.t("Create a new report")) + '">' + app.ic("plus", 14) + " " + app.esc(app.t("New Report")) + "</button>" +
            '<button class="nx-btn nx-btn-secondary nx-btn-sm" data-rx-tool="import" title="' + app.esc(app.t("Import reports")) + '">' + app.ic("download", 14) + " " + app.esc(app.t("Import")) + "</button>" +
            '<button class="nx-btn nx-btn-secondary nx-btn-sm" data-rx-tool="export" title="' + app.esc(app.t("Export my library")) + '">' + app.ic("share", 14) + " " + app.esc(app.t("Export")) + "</button>" +
            '<button class="nx-btn nx-btn-secondary nx-btn-sm" data-rx-tool="settings" title="' + app.esc(app.t("Reports Center settings")) + '">' + app.ic("settings", 14) + "</button>" +
            "</div>";
    }

    function reportCardHtml(app, c, r) {
        var key = r.key;
        var last = lastOpenedTs(app, key);
        var tags = tagsForReport(app, r, c);
        var fav = isFavorite(key), pin = isPinned(key), share = isShared(key);
        return '<div class="nx-rx-card" data-rx-report="' + app.esc(key) + '">' +
            '<span class="nx-rx-card-ic nx-ic-' + app.esc(r.color || (c && c.color) || "indigo") + '">' + app.ic(r.icon || (c && c.icon) || "file", 18) + "</span>" +
            '<span class="nx-rx-card-txt">' +
            '<span class="nx-rx-card-name">' + app.esc(app.t(r.title)) + "</span>" +
            '<span class="nx-rx-card-desc">' + app.esc(app.t(r.desc || "")) + "</span>" +
            (tags.length ? '<span class="nx-rx-card-tags">' + tags.map(function (t) { return '<span class="nx-rx-card-tag">' + app.esc(app.t(t)) + "</span>"; }).join("") + "</span>" : "") +
            "</span>" +
            '<span class="nx-rx-card-meta">' +
            (last ? '<span class="nx-rx-card-last">' + app.ic("clock", 11) + " " + app.esc(app.t("Opened {0}", [relTime(app, last)])) + "</span>" : "") +
            "</span>" +
    ...
    function customCardHtml(app, card) {
        var last = lastOpenedTs(app, card.base || card.key || "");
        var tags = card.tags || [];
        return '<div class="nx-rx-card is-custom" data-rx-custom="' + app.esc(card.id) + '">' +
            '<span class="nx-rx-card-ic nx-ic-' + app.esc(card.color || "indigo") + '">' + app.ic(card.icon || "file", 18) + "</span>" +
            '<span class="nx-rx-card-txt">' +
            '<span class="nx-rx-card-name">' + app.esc(app.t(card.title || "Untitled report")) + "</span>" +
            '<span class="nx-rx-card-desc">' + app.esc(app.t(card.desc || app.t("Custom report"))) + "</span>" +
            (tags.length ? '<span class="nx-rx-card-tags">' + tags.map(function (t) { return '<span class="nx-rx-card-tag">' + app.esc(app.t(t)) + "</span>"; }).join("") + "</span>" : "") +
            "</span>" +
            '<span class="nx-rx-card-meta">' +
            (last ? '<span class="nx-rx-card-last">' + app.ic("clock", 11) + " " + app.esc(app.t("Opened {0}", [relTime(app, last)])) + "</span>" : "") +
    ...
    function settingsDialog(app, rx) {
        ...
        app.showDialog({
            ...
            body:
                ...
                '<label class="nx-check"><input type="checkbox" data-nx-set-compact' + (p.settings.compact ? " checked" : "") + " /> " + app.esc(app.t("Compact report cards")) + "</label>" +
                '<hr class="nx-form-hr" />' +
            ...
            actions: [
                { label: app.t("Save"), variant: "primary", click: function (api) {
                    ...
                    var compact = api.el.querySelector("[data-nx-set-compact]");
                    if (empty) p2.settings.show_empty = empty.checked;
                    if (compact) p2.settings.compact = compact.checked;
                    writePrefs(p2);
                    api.close();
                    if (app.renderHub) app.renderHub();
                } },
    ...
    function bindSearch(app, rx) {
        var input = app.main.querySelector("[data-rx-search]");
        if (!input) return;
        input.addEventListener("input", function () {
            ...
            app.main.querySelectorAll(".nx-rx-cat").forEach(function (sec) {
                ...
                sec.querySelectorAll(".nx-rx-card").forEach(function (card) {
                    var key = card.getAttribute("data-rx-report") || card.getAttribute("data-rx-custom") || "";
                    var hay = (key + " " + card.textContent).toLowerCase();
            ...
            var count = app.main.querySelector("[data-rx-count]");
            if (count) count.textContent = app.num(visible, 0) + " " + app.t("reports");
        });
    }
    ...
    function shellHtml(app, key, opts) {
        ...
        return '<div class="nx-report nx-rx-report" data-rx-report-root>' +
            ...
            '<button class="nx-icon-btn nx-report-back" data-rx-back title="' + app.esc(app.t("Back")) + '">' + app.ic("chevron-left", 17) + "</button>" +
            '<span class="nx-report-crumb-txt">' +
            '<span>' + app.esc(app.t("Reports")) + "</span>" +
            '<span class="nx-report-crumb-sep">' + app.ic("chevron-right", 12) + "</span>" +
            '<span class="is-cur" data-rx-title>...</span>' +
            "</span></div>" +
            '<div class="nx-report-headrow">' +
            '<span class="nx-report-ic nx-ic-' + color + '" data-rx-ic>' + app.ic(icon, 18) + "</span>" +
            '<div class="nx-report-titles">' +
            '<div class="nx-report-title" data-rx-title2>' + app.esc(opts.title || key) + "</div>" +
            '<div class="nx-report-desc" data-rx-desc>' + app.esc(app.t("Loading report…")) + "</div>" +
            "</div>" +
            '<span class="nx-badge nx-badge-indigo nx-report-cat" data-rx-cat hidden></span>' +
            '<span class="nx-report-meta" data-rx-meta></span>' +
            '<button class="nx-icon-btn nx-report-refresh" data-rx-act="refresh" title="' + app.esc(app.t("Refresh report")) + '">' + app.ic("refresh", 16) + "</button>" +
            "</div></div>" +
            '<div class="nx-report-actions">' +
            '<button class="nx-raction" data-rx-act="filters" title="' + app.esc(app.t("Toggle filters")) + '">' + app.ic("filter", 14) + "<span>" + app.esc(app.t("Filters")) + "</span></button>" +
            '<button class="nx-raction" data-rx-act="save" title="' + app.esc(app.t("Save view")) + '">' + app.ic("save", 14) + "<span>" + app.esc(app.t("Save")) + "</span></button>" +
            '<button class="nx-raction" data-rx-pin="' + app.esc(key) + '" title="' + app.esc(app.t("Pin")) + '">' + app.ic("star", 14) + "<span>" + app.esc(app.t("Pin")) + "</span></button>" +
            '<span class="nx-raction-sep"></span>' +
            '<button class="nx-raction" data-rx-act="export-csv" title="' + app.esc(app.t("Export CSV")) + '">' + app.ic("download", 14) + "<span>" + app.esc(app.t("CSV")) + "</span></button>" +
            '<button class="nx-raction" data-rx-act="export-excel" title="' + app.esc(app.t("Export Excel")) + '">' + app.ic("download", 14) + "<span>" + app.esc(app.t("Excel")) + "</span></button>" +
            '<button class="nx-raction" data-rx-act="export-json" title="' + app.esc(app.t("Export JSON")) + '">' + app.ic("file", 14) + "<span>" + app.esc(app.t("JSON")) + "</span></button>" +
            '<button class="nx-raction" data-rx-act="print" title="' + app.esc(app.t("Print")) + '">' + app.ic("print", 14) + "<span>" + app.esc(app.t("Print")) + "</span></button>" +
            '<span class="nx-raction-grow"></span>' +
            '<button class="nx-raction" data-rx-act="fullscreen" title="' + app.esc(app.t("Fullscreen")) + '">' + app.ic("maximize", 14) + "<span>" + app.esc(app.t("Fullscreen")) + "</span></button>" +
            '<button class="nx-raction nx-raction-danger" data-rx-act="reset" title="' + app.esc(app.t("Reset")) + '">' + app.ic("rotate", 14) + "<span>" + app.esc(app.t("Reset")) + "</span></button>" +
            "</div>" +
            '<div class="nx-report-filters" data-rx-filterpanel hidden>' +
            '<div class="nx-report-filters-head"><span class="nx-report-filters-title">' + app.ic("sliders", 13) + " " + app.esc(app.t("Report Filters")) + "</span>" +
            '<span class="nx-report-filters-note"></span>' +
            '<button class="nx-icon-btn" data-rx-act="filters" title="' + app.esc(app.t("Close")) + '">' + app.ic("x", 14) + "</button></div>" +
            '<div class="nx-report-filters-body" data-rx-filters></div>' +
            "</div>" +
            '<div class="nx-report-body" data-rx-body>' +
    ...
    function renderFilterPanel(app, rx, meta) {
        ...
        html += '<div class="nx-rx-frow"><label class="nx-rx-flabel">' + app.esc(app.t("Company")) + "</label>" +
            '<select class="nx-input nx-rx-fselect" data-rx-field="company">' + selectOptions(opts.companies, "name", f.company) + "</select></div>";

        if (fields.indexOf("date") !== -1) {
            html += '<div class="nx-rx-frow"><label class="nx-rx-flabel">' + app.esc(app.t("Period")) + "</label>" +
                '<select class="nx-input nx-rx-fselect" data-rx-field="preset">' + presetOptions(f.preset) + "</select></div>";
            html += '<div class="nx-rx-frow nx-rx-frow-inline">' +
                '<label class="nx-rx-flabel">' + app.esc(app.t("From")) + "</label>" +
                '<input class="nx-input" type="date" data-rx-field="start" value="' + app.esc(f.start || "") + '" />' +
                '<label class="nx-rx-flabel">' + app.esc(app.t("To")) + "</label>" +
                '<input class="nx-input" type="date" data-rx-field="end" value="' + app.esc(f.end || "") + '" />' +
                "</div>";
        }
        ...
        fields.forEach(function (key) {
            ...
            if (key === "status") {
                html += '<div class="nx-rx-frow"><label class="nx-rx-flabel">' + app.esc(app.t(label)) + "</label>" +
                    '<select class="nx-input nx-rx-fselect" data-rx-field="status">' + optionList(STATUSES, f.status) + "</select></div>";
                return;
            }
            var list = key === "customer" ? opts.customers
                ...
                : [];
            html += '<div class="nx-rx-frow"><label class="nx-rx-flabel">' + app.esc(app.t(label)) + "</label>" +
                '<select class="nx-input nx-rx-fselect" data-rx-field="' + app.esc(key) + '">' + optionList(list || [], f[key] || "") + "</select></div>";
        });

        html += '<div class="nx-rx-frow nx-rx-frow-actions">' +
            '<button class="nx-btn nx-btn-primary nx-btn-sm" data-rx-apply>' + app.ic("refresh", 13) + " " + app.esc(app.t("Apply")) + "</button>" +
            '<button class="nx-btn nx-btn-secondary nx-btn-sm" data-rx-reset>' + app.esc(app.t("Reset")) + "</button>" +
            "</div>";

        panel.innerHTML = html;
        var note = app.main.querySelector(".nx-report-filters-note");
    ...
    function renderResults(app, rx, meta, payload) {
        ...
        if (charts.length) {
            ...
            charts.forEach(function (c) {
                html += '<div class="nx-rx-chart-card"><div class="nx-rx-chart-title">' + app.esc(app.t(c.title)) + "</div>" +
                    '<div class="nx-rx-chart" data-rx-chart="' + app.esc(c.key) + '"></div></div>';
            });
            html += "</div>";
        }
    ...
    function kpiHtml(app, k, currency) {
        ...
        if (k.delta !== null && k.delta !== undefined) {
            ...
            deltaHtml = '<span class="nx-kpi-delta"><span class="nx-badge ' + cls + '">' +
                app.ic(up ? "arrow-up" : "arrow-down", 12) + " " + app.num(Math.abs(k.delta), 1) + "%</span></span>";
        }
        return '<div class="nx-rx-kpi nx-kpi-sm nx-kpi-' + app.esc(k.color || "indigo") + '">' +
            '<div class="nx-kpi-sm-top">' +
            '<span class="nx-rx-kpi-ic">' + app.ic(k.icon || "circle", 13) + "</span>" +
            '<span class="nx-kpi-sm-label">' + app.esc(app.t(k.label)) + "</span>" +
            deltaHtml +
            "</div>" +
            '<div class="nx-kpi-sm-value">' + value + "</div>" +
            (k.delta_label ? '<div class="nx-kpi-sm-sub">' + app.esc(app.t(k.delta_label)) + "</div>" : "") +
            "</div>";
    }
    ...
    function renderChart(app, rx, el, c, currency) {
        ...
        var config = {
            ...
            tooltip: { theme: dark ? "dark" : "light", y: { formatter: formatter } },
            xaxis: { categories: c.categories || [], labels: { style: { colors: dark ? "#aab3c5" : "#5b6472" } } },
            yaxis: { labels: { formatter: formatter } }
        };

        if (isBar) config.plotOptions = { bar: { horizontal: true, borderRadius: 4, barHeight: "62%" } };
        if (isColumn) config.plotOptions = { bar: { horizontal: false, columnWidth: "55%", borderRadius: 4 } };
        if (isDonut) {
        ...
        try {
            var chart = new window.ApexCharts(el, { series: series, ...config });
            chart.render();
            rx.charts.push(chart);
        } catch (e) {
            el.innerHTML = '<div class="nx-rx-chart-fallback">' + app.esc((e && e.message) || "Chart error") + "</div>";
        }
    }
